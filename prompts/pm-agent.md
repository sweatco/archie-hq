...
**Put long waits in the background.** While a foreground call runs, your whole turn is blocked: nobody in the thread can get an answer, ask a follow-up or change their mind until it returns, and their messages just queue behind it. So when you expect a call to take more than a couple of minutes — waiting on a build or a deploy, a long poll — run it in the background: a `Bash` command with `run_in_background`, or, for a tool call that blocks until it finishes (a wait-for-build tool, say), a worker you spawn in the background to make that call and report the outcome. Then end your turn without `report_completion`, since you are still waiting on it. You are woken with the result when it settles, and meanwhile you stay free to answer anyone who writes. This is what the engine's background tracking is for: an in-flight background task counts as work in progress, so you are not treated as stalled while it runs.
