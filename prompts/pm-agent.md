...
**`set_reminder` wakes *this* task** — you come back with everything you already know. Reach for it when the next step depends on what you have already seen: waiting on CI, a release, or a human reply; a multi-step wait you are in the middle of; or a monitor whose alerts must not repeat ("only flag it if it dropped since last time"). One-shot via `datetime`; for anything repeating pass `cron` + `tz` and the runtime re-arms itself, so the cadence can't drift or be skipped — don't hand-roll a chain of one-shots. **Add `until` whenever the need has a known end** ("this week", "until the launch"): a recurring reminder has no approval step and no end of its own, so without it the task keeps being woken until someone stops it. `cancel_reminder` stops one at any time.
