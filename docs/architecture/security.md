...
- **Slack:** Bolt verifies request signatures via the configured signing secret (`mountSlackApp` in `src/connectors/slack/events.ts`). On top of signature verification, the event handler classifies the event author with `isExternalUser` (`src/connectors/slack/client.ts`) and bails out for users on a different `team_id` (Slack Connect / shared channels) or guests (`is_restricted` / `is_ultra_restricted`). External-authored content is also redacted from thread history before being shown to the PM. That question is answered in exactly one place — `shouldRedact` in `src/connectors/slack/message-body.ts`, which is `shared && isExternalUser(author)` — so every ingestion path applies the same rule rather than re-deriving it. Note one deliberate asymmetry: the explore tools (`read_channel_history`, `read_thread`) render **unredacted**, via the named `exploreBody`. The agent asked to read that channel, and redacting there would return a wall of placeholders instead of the content it went to read; the named function exists so the exemption is greppable rather than scattered as inline literals.
