...
- **Slack:** Bolt verifies request signatures via the configured signing secret (`mountSlackApp` in `src/connectors/slack/events.ts`). On top of signature verification, the event handler classifies the event author with `isExternalUser` (`src/connectors/slack/client.ts`) and bails out for users on a different `team_id` (Slack Connect / shared channels) or guests (`is_restricted` / `is_ultra_restricted`).
  ...
  **What reaches the transcript is decided by the author alone.** `shouldRedact` in `src/connectors/slack/message-body.ts` is `!isTrustedIngestAuthor(author)`, and that predicate trusts only a positively verified home-workspace human — or an automation whose id the operator listed in `SLACK_TRUSTED_AUTOMATION_IDS`. Everything else is replaced by the redaction placeholder: a guest, another workspace's member, an author carrying no `team_id` at all, an author whose `users.info` lookup failed, and any un-allowlisted bot or app user. Every ingestion path routes through the one predicate rather than re-deriving it.

  The verdict deliberately does **not** consult whether the channel is currently shared with an external workspace. A transcript outlives that state, and the memory runtime extracts it later from disk, so "was this channel shared at capture time" is the wrong question to hang confidentiality on: a contractor in an ordinary private channel and a member of a group DM are no more accountable for their text than a Slack Connect member. `isChannelShared` remains a display-time signal, used for the ephemeral Slack Connect warnings and nothing else. The trade this makes is deliberate: an un-allowlisted internal automation loses its text from the transcript, which is a quality cost paid to close a laundering path — a relay app (GitHub, Jira, an RSS bridge) posts text written by someone outside the workspace entirely.

  Three render paths are deliberately **unredacted**, and each is safe by a *different* upstream drop rather than by redaction — so they are named functions in the render module, not inline literals, and the exemptions are greppable in one place. `exploreBody` (`read_channel_history` / `read_thread`): the agent asked to read that channel, and redacting would return a wall of placeholders instead of the content it went to read. `pinBody`: the two-principal pin gate has already dropped anything externally authored or externally pinned, so nothing external can reach it. `rawMessageBody` (the edit handler and channel-message trigger dispatch): the edit handler drops an edit whose author cannot be classified, and trigger dispatch drops an app post from a foreign workspace before rendering.

  Startup fails closed too: `initSlackClient` **throws** when `auth.test` does not return both a bot user id and a home `team_id`. Every trust decision reads the home team, so a client that came up without one would classify nobody — the deployment failing visibly beats it looking healthy while ignoring Slack.

  Two residual paths write to `knowledge.log` without passing the author rule, and it is worth knowing which. The **ingestion floor** in `fireTrigger` (`src/system/trigger-scheduler.ts`) covers the case where `fetchSlackThread` dropped the triggering message; because that only happens for a payload carrying **no identity at all** — no `user` and no `bot_id` — the floor records the message's `msg:<ts>` id with the redaction placeholder rather than its text. The larger one is **agent-mediated relay**: an explore read, a pin index or a channel canvas hands an agent unredacted channel content, and the agent may then quote it into a finding. That is not a fail-open bug in this layer — the agent was authorised to read it — but it does mean the transcript's confidentiality guarantee is about what Archie *ingests*, not about everything an agent can put there. GitHub webhook bodies (`appendGitHubEvent`) are unfiltered for the same reason: they carry no Slack identity to classify. Any consumer that republishes a public task's transcript beyond the task must apply its own policy to these paths — `visibility` classifies the Slack conversation, not every byte in the log.

  Two operational notes on the fail-closed posture. A persistent `users.info` failure (revoked `users:read`, sustained rate limiting past the SDK's retries) redacts every affected message — loudly, via a warning per failed lookup — rather than trusting what it cannot classify; the redaction is permanent for messages ingested during the outage, which is the accepted cost of an append-only log. And the operator HTTP API creates its tasks as `public` on the standing assumption that the API is a network-restricted operator surface; it carries no authentication of its own, so a deployment that exposes the port must front it with access control or that assumption is void.
