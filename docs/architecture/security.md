...
**Some paths are granted write-only** — present in `allowWrite` without a matching `allowRead`. `CACHES_DIR` (`$ARCHIE_WORKDIR/caches/`, the shared package-manager cache) is one; a trigger-fired task's `$ARCHIE_WORKDIR/triggers-data/<trigger-id>/` is another, see [Triggers](triggers.md#persistent-per-trigger-directory). Check the `allowWritePaths` grant sites in `src/agents/spawn.ts` for the current set rather than trusting a count here. Two consequences follow for any such path. First, it is fully usable — including from `Bash`. That is worth stating because the opposite is the intuitive reading, and limitation 1 below used to assert it: measured under bwrap 0.11.0 with this exact config, an agent can `cat`, `ls` and write to the path from the shell, and the writes persist to real disk. `denyRead` emits its `--tmpfs` **before** the `allowWrite --bind`, so the bind sits on top and survives; the parent stays opaque while the granted subtree punches through. Adding the path to `allowRead` as well changes nothing observable — so the write-only form is a free choice here, not a constraint. Second, `assertReadable` (`src/agents/artifacts.ts`) validates against the union of both lists rather than `allowRead` alone, so the in-process artifact tools are not stricter than the sandbox they mirror; without that, an agent could write a file into either path and then be refused when sharing it. Note that this makes a write-only grant a decision about outward flow as well as about writing: whatever an agent may write there, it may also hand outward via `share_artifact`. For `CACHES_DIR` that means package tarballs any agent on any task wrote — low-value, but a cross-task read surface the union opens rather than inherits. Weigh it when adding a shared write-only grant.
