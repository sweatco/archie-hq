...
An MCP server can declare execution tiers and Slack user group restrictions in the `archie` block beside its connection config (see [Tool Approvals](tool-approvals.md)). A `PreToolUse` hook applies the current policy to every agent mounting that server, including the PM. `deny` always refuses. `requesterGroups` checks verified human task authority before either an `allow` or `ask` call. `ask` posts an engine-rendered per-call approval prompt; `approverGroups`, when configured, restricts its resolution to eligible Slack members. Requester-restricted tasks support one human author and permanently lose that authority on another author's or API/GitHub input.

Protected grants bind task/request identity, tool/arguments and policy revision. Policy and memberships are checked at approval and consumption; group cache freshness is bounded to 60 seconds and group events invalidate it. The grant is removed and atomically saved before execution; storage or verification failure denies the call. Verified Slack actions supply identity, while caller-supplied API metadata cannot satisfy approver groups. Servers without `archie.access` retain their legacy behavior. The API still needs ingress protection. These checks cover Archie-hosted plugin calls; they do not authorize other MCP clients or replace server-side credential controls.
