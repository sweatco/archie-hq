...
An MCP server can declare execution tiers and Slack user group restrictions in the `archie` block beside its connection config (see [Tool Approvals](tool-approvals.md)). A `PreToolUse` hook applies the current policy to every agent mounting that server, including the PM. `deny` always refuses; `allow` runs ungated. `ask` posts an engine-rendered per-call approval prompt; `approverGroups`, when configured, restricts its resolution to eligible Slack members. Anyone or an automated task may propose a call, but only the verified approver can authorize its execution.
