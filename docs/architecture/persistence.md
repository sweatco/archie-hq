...
Because a taskId can arrive untrusted from the HTTP API, both the writer (`appendUsageRecord`) and the reader (`aggregateTaskUsage`) validate it with `isSafeTaskId` — the canonical `generateTaskId` shape, a single path segment with no `..` — before any path is built from it, so a taskId can never traverse outside the sessions/ root; an unsafe id is a silent no-op on write and an empty report on read.
