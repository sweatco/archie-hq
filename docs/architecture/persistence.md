...
- The within-nonce cumulative-vs-delta reducer was confirmed on the live boot: successive result events within one query() call are cumulative (monotonic non-decreasing), so `max` is correct. The reducer is injectable, so if a future SDK ever emitted per-turn deltas (a decrease within a nonce) the fallback to `sum` is a one-line change confined to `reduceNonceCost`. For any nonce with a single result event, `max == sum`, so the choice cannot produce a wrong number.
