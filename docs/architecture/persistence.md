...
Archie joins two data sources at report time. Token counts come from SDK transcripts. Cost comes from SDK `result` events and is unavailable when a turn ends without one.

### The `shared/usage.jsonl` writer

On every SDK `result`, `spawn.ts` queues one `TaskUsageRecord` for `shared/usage.jsonl`. Writes are serialized per task and fail safely, so telemetry cannot block or break the agent loop. Writer and reader both reject non-canonical task IDs before filesystem access.

### The `query_nonce` cost model

One SDK session can span several independent `query()` calls, so `session_id` cannot delimit cost windows. `spawn.ts` instead assigns one `query_nonce` to each call. Read-time aggregation is:

- take the maximum cumulative `total_cost_usd` within one nonce;
- sum across nonces, grouped by agent for the per-agent breakdown.
...
`get_task_usage` is a PM-only tool. It recursively scans agent and subagent transcripts, deduplicates assistant messages by ID, excludes synthetic and journal records, and rolls subagent tokens into their parent agent. It reports SDK cost without a local price table. Missing cost remains `unavailable`, never zero, and the report discloses when fewer turns have cost records than transcript records.

### Caveats (documented, not corrected)

- SDK cost is an SDK estimate, not actual Anthropic billing; subscription authentication can diverge.
- Cache-write tokens remain one bucket. Archie does not model ephemeral-cache duration or inference-region multipliers.
