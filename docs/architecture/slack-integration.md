...
- **Deliver.** Each rendered line goes through a single sink, `Task.onStatusRendered`, which fans it out to all surfaces so the status can be observed with or without Slack: (1) a `status` event on the in-process event bus → SSE → the **CLI** renders the same "Archie is …" line live (a spinner above the message input — `TaskDetail.tsx`); (2) the unified logger so it's visible in headless / dry-run / `npm run dev` output; (3) the **Slack** assistant-thread indicator (best-effort, every linked non-muted Slack thread). A CLI-only task has no Slack channel, so the Slack push is a no-op there and the CLI/log surfaces are how you test it.
- **Debounce & clear.** Pushes are debounced (~1s) and de-duplicated so no surface is spammed and the indicator doesn't flicker between turns. It is cleared when the PM posts to the user (`logOutgoingMessage` → Slack also auto-clears on a posted reply) and on task `stop()`/`complete()`. Activity is in-memory only — never persisted — so a restart never resurfaces a stale "working…"; Slack's built-in ~2-minute timeout is the final backstop. The whole feature is gated by `ARCHIE_LIVE_STATUS` (default on).
