...
Group DMs get the same **channel extras** as real channels wherever Slack supports them: the canvas scan (`getChannelCanvasTabs`), shared-channel detection (`isChannelShared` via `conversations.info`), and the ambient live status indicator. The external/guest-author bail-out (`isExternalUser`) runs on the group-DM path just as it does elsewhere — an external author never triggers a task — and ingestion redaction applies the same author rule here as anywhere else, since it never consults the conversation's shared state. A group DM also counts as a **private** channel for task visibility, so a task seeded in one is private.
