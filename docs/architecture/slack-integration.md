...
- **Debounce, keepalive & clear.** Pushes are debounced (~1s) and de-duplicated so no surface is spammed and the indicator doesn't flicker between turns. Because Slack auto-clears a status after ~2 minutes if nothing refreshes it, a keepalive re-asserts the current status every ~90s — without it, any long, quiet stretch (a `web_research` call, or a single agent working a while without a status change) would lose the indicator mid-work. Each beat **alternates the trailing ellipsis glyph (`…`/`...`)** so the re-sent value is distinct (Slack ignores identical re-sends) while rendering the same — no visible flicker. It is cleared when the PM posts to the user (`logOutgoingMessage` → Slack also auto-clears on a posted reply) and on task `stop()`/`complete()` (which also stops the keepalive). Activity is in-memory only — never persisted — so a restart never resurfaces a stale "working…". The whole feature is gated by `ARCHIE_LIVE_STATUS` (default on).
