...
A muted thread (`SlackChannel.muted = true`, set by the PM's `mute_channel` tool) is unmuted by an `@mention` and otherwise skipped. In DM channels, any inbound message also unmutes — there is no `@mention` path in a DM, so a DM message is treated as the equivalent. `mute_channel` mutes a single channel — the one the PM names, or the task's `default_channel` if omitted; it refuses to mute DM channels (channel IDs starting with `D`) up front, but the DM-as-unmute rule above is the backstop that recovers any legacy task whose DM channel was muted before this restriction existed. Title generation runs as a fire-and-forget Haiku call after the first append; for DM-rooted tasks the resulting title is pushed to Slack via `assistant.threads.setTitle` (see `src/connectors/slack/title.ts`). External event authors cannot start work. In fetched history, verified external authors are redacted. Messages are processed chronologically. After the Slack SDK exhausts its lookup retries, a historical author whose workspace identity still cannot be verified is rendered as an unresolved-author placeholder and the watermark advances; the current inbound actor is verified separately before any task processing and remains fail-closed.
...
Every task has a monotonic `public | private` visibility. Public Slack channels, including Slack Connect channels, create public tasks. Private channels and DMs create private tasks. Security-sensitive thread ingestion and edit handling require a successful live channel lookup; a lookup error aborts that ingress instead of being persisted as private. Best-effort display surfaces may still fall back to a private-looking channel stub. CLI tasks are public. A task can downgrade from public to private but never upgrade. The downgrade is flushed before any content or watermark from that private source is written; a failed flush restores the live value and aborts ingestion so a later attempt can retry. Message triggers inherit the source thread's strictly verified visibility, while scheduled triggers derive it from the live delivery channel. Legacy tasks without the field load as private.
...
Each Slack channel entry stores a `last_processed_ts` timestamp. The eyes-reaction acknowledgment in `handleSlackEvent` uses it to remove the previous "eyes" reaction before adding one to the new message, so only the most recent inbound message ever shows the indicator. `task.append(thread)` advances it after each ingested or redacted message and skips messages it has already absorbed. An unresolved historical author is redacted and advances the watermark; the current inbound actor is separately fail-closed and is never advanced when unresolved.

Automation status is separate from workspace affiliation. Archie's own root message is trusted. Other bot/app messages enter task history only when their bot ID or app user ID is listed in `SLACK_TRUSTED_AUTOMATION_IDS` and Slack verifies the home workspace. Unlisted automation is redacted. Automation whose identity remains unresolved after Slack's retries is rendered as an unresolved-author placeholder and does not wedge later messages; verified external automation is redacted.
...
Button clicks are handled by Bolt action handlers in `src/connectors/slack/events.ts`. Every authorization action resolves the actor before changing state; external users, guests, missing actors, and lookup failures are acknowledged and ignored. A lookup failure posts a private retry message to the clicker. An external or guest click posts a private explanation and annotates the original card as waiting for a workspace member while preserving its approval controls. For an internal actor, the handler updates the original message and calls the corresponding `Task` method. The same guard covers approvals and denials for edit mode, max mode, research budget, merges, and triggers.
...
- **Shared channels (Slack Connect)** — `isChannelShared` (60s TTL cache) flags external-shared channels and preserves a previously observed `shared=true` result across transient API failures. A first lookup failure still fails open to non-shared because author and channel-confidentiality checks are independent. `sendSharedChannelWarnings` posts ephemeral notices once per (thread × user): a general shared-channel heads-up to internal participants, and a provenance notice when forwarded content is not verified as internally authored.
