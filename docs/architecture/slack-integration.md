...
2. **`message`** -- Fires for thread replies, DM messages, and top-level channel posts in channels a trigger watches. Slack delivers these via four subscribed event types — `message.channels` (public channels), `message.groups` (private channels), `message.im` (1:1 DMs), and `message.mpim` (group DMs) — all of which Bolt surfaces as a single `message` event.
   ...
   **App posts reach triggers but never wake a task.** `mayWakeTask` refuses any event carrying a `bot_id`, or the `bot_message` subtype — both signals, because Slack sets that subtype only for incoming-webhook and `as_user: false` posts while an app calling `chat.postMessage` arrives with `bot_id` and no subtype at all. Waking a task spends a PM turn, and CI, Jira, Datadog and GitHub-app posts arrive at volumes no human sends. Task creation additionally requires the fetched thread to carry at least one visible message, so a payload with no author and no body cannot seed a task on an empty knowledge log.
...
- `src/connectors/slack/message-body.ts` — **the one place a Slack message becomes agent-facing text**: `renderMessageBody` plus the named entry points every path uses (`messageBody` for ingestion, `exploreBody` for explore reads, `pinBody` for the pin index, `rawMessageBody` for a raw Bolt payload) and `shouldRedact`, the single answer to the redaction question
