...
Every user-facing message carries a small grey footer: `task-<id> · <models>`, where `<models>` is the **distinct set of models the task has actually used** — PM first, then each spawned specialist — joined with ` + ` (e.g. `task-20260626-2130-a3f9k2 · Opus 5 + Sonnet 5 (1M)`). As more agents join, the set grows. It is built once per send by `Task.buildUserFooter()` → `collectModelsUsed()`, which prefers the **concrete model that actually served each agent's latest turn** — recorded per `assistant` message (and seeded at `init`) via `Task.recordResolvedModel()` (so `opus` shows as its real version, e.g. `Opus 5`, and a max-mode turn shows the swapped model, e.g. `Fable 5`). Because the footer is built after the turn completes, this is the model that produced that turn's output; it falls back to the configured alias (`resolveAgentModel`, shared with `spawn.ts`) only before an agent's first turn. Labels are then derived by `modelDisplayLabel` in `src/agents/model-label.ts`, which **parses** any model id generically — drops the `claude-`/`anthropic/` prefix, capitalises the family, dots up to two numeric version segments, drops the date, and renders the `[1m]` 1M-context marker as `(1M)` — so a brand-new model version renders correctly with no code change (there is no per-version lookup table). Delivered to both surfaces:
