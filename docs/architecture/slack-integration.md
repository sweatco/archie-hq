...
While a task is being worked on, Archie drives Slack's assistant-thread loading status — the shimmering "**Archie is …**" line under the composer — via `assistant.threads.setStatus`. It is the progress sibling of the title pipeline: same `client.assistant.threads.*` accessor, same `channel_id` + `thread_ts`, wrapped best-effort in `src/connectors/slack/status.ts` (errors and an uninitialised client are logged and swallowed; honours dry-run). Slack auto-prepends the app name, so the status string is always a verb fragment (`"is checking mobile and backend…"` renders as "**Archie** is checking mobile and backend…"). Since the 2026-03 platform change `setStatus` accepts `chat:write` as well as `assistant:write`, so it works in regular channel threads as well as DM/assistant threads — Archie sets it on every linked, non-muted Slack thread.
...
- **Capture.** The per-agent SDK loop in `src/agents/spawn.ts` calls `task.noteActivityFromEvent(agentId, event)` on every event, alongside the existing logging hook. `deriveActivity` (`src/agents/activity.ts`) maps each `tool_use` block (`block.name`, `block.input`) to a short first-person fragment — e.g. `Read`/`Grep` → "digging into the backend", `create_pull_request` → "opening a pull request", `mcp__rollbar__*` → "checking the error reports". Internal plumbing (delegation, `post_to_user`, owner assignment, reminders) maps to `null` and never surfaces. Agent active/idle transitions flow through `Task.updateAgentState`, which already knows who is mid-turn.
...
- **Push & clear.** Pushes are debounced (~1s) and de-duplicated so Slack is never spammed and the indicator doesn't flicker between turns. The indicator is cleared when the PM posts to the user (`logOutgoingMessage` → Slack also auto-clears on a posted reply) and on task `stop()`/`complete()`. Activity is in-memory only — never persisted — so a restart never resurfaces a stale "working…"; Slack's built-in ~2-minute timeout is the final backstop. The whole feature is gated by `ARCHIE_SLACK_STATUS` (default on).
...
- `src/connectors/slack/status.ts` — `assistant.threads.setStatus` wrapper (best-effort) + `ARCHIE_SLACK_STATUS` gate for the live status indicator
- `src/tasks/status.ts` — `TaskStatusController`: composes the single first-person status line from active agents (PM precedence → single specialist → aggregated domains), debounced
