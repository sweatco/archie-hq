...
**Two principals are gated, not one.** A pin has an author and a pinner, and they are usually different people. Both must classify as internal (`isExternalUser`); either being external drops the item, and either being unclassifiable fails closed — an already-vetted entry is retained, a new one is skipped and retried at the next scan. An **app-posted pin is refused**, before either principal is resolved: a relay app posts as an in-workspace user, so classifying its author establishes only that the relay is internal and nothing about where the content came from. Archie's own posts are the single exception — there is no laundering question about content this system wrote itself, and a human pinning an Archie message is deliberate curation. Identity is `bot_id`, which Slack assigns and a message cannot claim for itself; the bot's own user still goes through the same classifier, so an unresolvable lookup fails closed like anyone else's.
