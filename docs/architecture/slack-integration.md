...
**Two principals are gated, not one.** A pin has an author and a pinner, and they are usually different people. Both must classify as internal (`isExternalUser`); either being external drops the item, and either being unclassifiable fails closed — an already-vetted entry is retained, a new one is skipped and retried at the next scan. An **app-posted pin is refused outright**, before either principal is resolved: a relay app posts as an in-workspace user, so classifying its author establishes only that the relay is internal and nothing about where the content came from.
