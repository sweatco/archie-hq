...
**Model and effort.** The PM's prompt requires a model on every spawn: `sonnet` for coding, research and analysis, `opus` only when the work clearly needs it, `fable` for coding workers while max mode is approved. An unset model inherits the PM's own, which is the expensive accident the rule exists to prevent. **Effort is not settable per spawn** — a generic worker inherits the session's effort, so effort variety needs an agent definition file.
