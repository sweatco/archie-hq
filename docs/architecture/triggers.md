...
- **`Bash` is not a usable route to it.** `denyReadPaths` includes `$ARCHIE_WORKDIR`, and under bubblewrap the limitation in [Known Sandbox Limitations](security.md#known-sandbox-limitations) means the `denyRead` tmpfs lands after the `allowWrite` bind, with no `allowRead` entry to restore even read access. That mechanism is bwrap-specific and has not been characterised for the macOS `sandbox-exec` backend, so treat "shell cannot reach it" as the deployed-Linux behaviour rather than a proven invariant everywhere. Either way agents are told to use `Read`/`Write`/`Edit`, which the in-process PreToolUse hooks gate instead, so they do not waste a turn on `ls`.
