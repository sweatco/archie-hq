...
**A message fire hands over the message.** `buildTriggeringMessageBlock` puts the matched message — its text, its author's Slack id, its channel and its ts — into the seed as a `<triggering_message>` block, ahead of the instruction. It is framed the way every other block of user-authored text in the system prompt is framed (see `buildChannelPinnedMessagesSection`): untrusted input, data rather than instructions, and a Slack identity proves nothing about who someone is. That framing carries more weight here than elsewhere, because the trigger's own filter is what decides which text reaches an agent, so a channel member can aim text at this block deliberately. Long messages are cut at 2000 characters with a pointer to the thread.
