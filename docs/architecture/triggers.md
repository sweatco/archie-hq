...
Because those stores are otherwise refreshed only by inbound Slack events, a schedule fire refreshes them for the home channel before it wakes the task — and refreshes the canvas with `{ announce: false }`. A canvas scan announces adoption as a new top-level channel message, and posted here it would arrive ahead of the automation's own result and root a bot-authored thread. A scan that may not announce **queues** its notices in the channel store instead — `ChannelStore.pendingAnnouncements`, persisted per channel under `$ARCHIE_WORKDIR/slack/channels/<id>.json` — and the next scan that may post flushes them first, ahead of the TTL check so a quiet channel does not sit on them. The refresh itself is never deferred — abandoning the write was the first attempt and it kept a canvas whose creator had just been reclassified as external in the store, markdown and all, for exactly the one fire the scan was run for.
