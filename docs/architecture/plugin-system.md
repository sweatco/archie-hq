...
`archie.access` optionally restricts human access by Slack user group: `access.default` applies to the server, and `access.tools.<bare method>` overrides individual `requesterGroups` / `approverGroups` lists. Omitted fields inherit; supplied lists replace; empty lists are invalid. Requester checks apply even to `allow` tools. Policies are reread for each mounted plugin invocation, including policies added during an active session. See [Slack user group access](tool-approvals.md#slack-user-group-access) for the schema, verified identity requirements and rollout steps. These fields are Archie metadata, stripped before SDK connection setup.
