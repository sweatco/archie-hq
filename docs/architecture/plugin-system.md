...
Reading a mounted skill's *files*, as opposed to loading it, needs a sandbox grant, and spawn issues one: it adds each agent's mounted core skill directories to `allowReadPaths`, because their real path is under `/app`, which `buildSandboxConfig` denies. Loading was never affected — `Skill` is gated by neither enforcement layer. See [security.md](security.md#filesystem-isolation) for what was measured without the grant.
