...
Archie-hq ships five skills of its own in the repo's top-level `skills/` directory rather than inside any plugin: `channel-canvas`, `self-awareness`, `thread-conduct`, `triggers` and `trigger-continuity`. Which agents receive them is declared in exactly one place — the `CORE_SKILL_MOUNTS` manifest in `src/agents/core-skills.ts` — keyed by agent **track**, one of `pm`, `repo` or `plain`. The first four sit on the `pm` track alone. `trigger-continuity` sits on **all three**, because the per-trigger directory it describes is granted to every agent on a trigger-fired task, not just the coordinator — so it is the one core skill that reaches repo and plugin agents. Giving a skill a second audience is a line in that manifest; it does not require a second directory.
...
Two limits are worth knowing before giving a core skill a new audience. A mounted skill is loadable through the `Skill` and `Read` tools, but its files are **not** readable from sandboxed `Bash` in the production image, because bubblewrap resolves the symlink and `/app` is in `denyRead`. And the four PM-track skills are written in the PM's voice and instruct PM-only MCP tools (`post_to_channel`, `mute_channel`, `read_thread`, `fetch_slack_reference`, `report_completion`), which are attached only on the PM — so widening one of *those* needs prompt and tool work too, not just a manifest entry. `trigger-continuity` is the counter-example and shows what a multi-track core skill costs: it was written track-neutrally from the start, names no MCP tool at all, and assumes nothing about talking to users or owning a channel.
