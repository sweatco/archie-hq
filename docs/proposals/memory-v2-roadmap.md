...
> **Status:** Phase 1 shipped (`memory-v2-phase1`, `memory-v2-fixes`, `memory-v2-injection-bounds`, `memory-v2-selection-sensor` all landed). Phase 2 proposed as `memory-v2-pull-and-eval`, which also carries the injection-enablement gate and flip. Injection still default-off in prod. Later phases are proposals, not commitments — each needs its own OpenSpec change.
> **As-built:** [`docs/architecture/memory.md`](../architecture/memory.md) · **Spec:** [`openspec/specs/memory-layer/spec.md`](../../openspec/specs/memory-layer/spec.md) · **External evidence:** 2026-07 verified research sweep (digest untracked alongside this file; conclusions baked into `memory-v2-pull-and-eval`'s design)
...
| **Phase 2** (proposed: `memory-v2-pull-and-eval`) | pull: read tools for all agents; injection enablement (gated) | snapshots + pull sensor + eval harness (mechanical **and functional** — real impl scored on a benchmark + prod-derived question set) + enablement gate + the prod-QA loop |
...
## Phase 2 — Pull + measure (proposed: `memory-v2-pull-and-eval`)

The next step. One change, everything the current state can honestly support:

**Runtime tools (appropriate to the current state — read-only).** `search_memory`, `read_entity`, `read_task_summary`, `grep_task_log` for all agent tracks behind `ARCHIE_MEMORY_TOOLS` (default off, independent of the injection flag). Writes stay funneled through the extractor. Search is lexical with the selector's own tokenization — pull-vs-push deltas then indict budgets, not scorer skew. File/grep read granularity is where the field converged: Anthropic's GA memory tool is six file operations with no search command, Letta v2 replaced embedding archival search with `open_file`/`grep_file`, and three independent 2026 studies have grep/file agents matching or beating dense retrieval (evidence: change design D1/D3).
...
**Spec cleanup.** memory-layer spec loses retired-feature residue (org.md backfill narration, the removed Slack learnings post, resolved open questions, stale summary paths); the "no pull tool" non-goal is superseded.

Rollout order: run the enablement gate → flip injection → flip tools with or after injection (without the injected `<entity_index>` agents have no catalogue of what's pullable, and early pull telemetry would misread as "no demand").
