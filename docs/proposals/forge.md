...
- **Revise** (post-ship, operator-invoked: `/forge revise <request>`) — a requested change to a shipped run's PR goes through `forge-revise`, never through hand-edits: request-mode implement (the blind reviewers run, told the request is sanctioned scope on top of the plan) → one full QA pass (failures impasse to the operator — they are steering, so no auto route-back) → docs delta → PR-body and plan-record refresh. Works cross-session: the run identity (brief, ACs, design, verification plan) reconstructs from the `docs/plans/` record when the original session is gone. This is the deliberate manual-steering loop for fine-tuning the process; the future issue-driven phase absorbs it (review feedback arrives as events and routes through the same machinery without an operator relay).
...
The zero-footprint mode from v1 survives, and it is the purest workflow of all — stateless fan-out, no gates, nothing written. It also reviews the **local working tree as-is**: `/forge review` / `/forge qa` without a PR number run the same machinery on the current checkout's state — uncommitted and untracked changes included, snapshot-copied into the isolated worktree so the operator's checkout is never touched — diffed against base, with intent derived from commit messages or taken verbatim from the operator (`/forge qa "intended behavior"`). `/forge qa <n>` is the QA-only alias for PRs.

Review mode is also **direction-aware**: a fourth grounding lens reads the open issues and PRs, `docs/proposals/`, and `docs/plans/` (which every Forge run feeds) to establish what is in flight, planned, or explicitly uncommitted — and the claims reviewer routes scope judgments into a dedicated **`scopeQuestions`** channel, separate from findings: work that is competently built but possibly premature (an abstraction with no second consumer, groundwork for a future the maintainers haven't committed to, territory colliding with in-flight direction) surfaces as questions with evidence, not verdicts. The conductor asks the operator these questions at the report gate — the descope call is always human — and approved scope requests ride into the submitted review. In the future issue-driven phase, scope questions are exactly what gets posted to the PR thread for the maintainer to rule on: Forge proposes the descope, the human disposes. `forge-review` reviews and QAs an existing PR **without taking it over**: an ephemeral worktree isolates the checkout; fact-checked lenses (PR context, diff mapper, base-drift check) ground it; an agent derives the intent and numbered ACs from the PR autonomously with every assumption flagged (the PR's own "couldn't verify" admissions become ACs); the review ring (claims-vs-diff + adversarial bug hunter with mutation-checked tests, skipped in `qa-only`) and the blind QA ring run against the derived contract; the worktree is torn down. Unlike `forge-run` it never impasses — a dead agent or unavailable infra degrades into an explicit `gaps` entry in the report rather than a stop. The conductor renders the report in chat, iterates via relaunches with the operator's corrections, and — only on the explicit go — submits the review to GitHub with honest attribution and line-anchored comments. Follow-up rounds relaunch with the previously reviewed SHA and findings, ruling each fixed / unaddressed / regressed. Review mode is exempt from one-run-per-session (it writes nothing); its only shared resource with an active run is the docker boot during live QA.
