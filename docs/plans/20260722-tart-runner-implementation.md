...
- [x] Build and debug a deterministic iOS app from source through Archie in a real Tart VM, including XCTest, LLDB value inspection, crash evidence, artifact collection, and verified cleanup.
...
## 2026-08-20 Source-to-debug iOS canary

- Added `fixtures/ios-runner-app`, a committed dependency-free Xcode project that builds for Simulator without signing. Its shared scheme runs one unit test and one UI accessibility test; the app exposes a stable Swift LLDB breakpoint/local-value contract and a deliberate crash marker.
- Added the compiled `runner:ios-full-cycle-e2e` harness. It requires a destructive opt-in, dedicated runner controller, clean exact source commit, exact Simulator runtime, project/workspace and product contract, and bounded timeout/debug hold.
- The harness verifies a source manifest on both sides of `RunnerManager.sync`, runs `xcodebuild build-for-testing` and `test-without-building`, validates machine-readable build/test summaries, launches the built app, captures a screenshot, inspects a local value at a real breakpoint, records a backtrace and detach, captures a deliberate crash through a marker-bearing unified log plus a preferred `.ips` or explicit log report, collects both `.xcresult` bundles and evidence, deletes the Simulator, releases the VM, and verifies backend deletion.
- The canary deliberately avoids `RunnerManager.initialize()` so its disposable workdir cannot perform global orphan deletion against another controller. Cleanup errors are aggregated and make the run fail.
- Local Xcode 26.5 / iOS 26.5 validation passed: two tests, accessibility identifiers `runner.nonce`, `runner.debug`, and `runner.crash`, a 402×874 PNG, LLDB breakpoint plus `nonce = "ARCHIE_FULL_CYCLE_NONCE"`, eight-frame backtrace and detach, and an `.ips` report paired with the `ARCHIE_FULL_CYCLE_CRASH` unified-log marker.
- The compiled harness passed against an isolated local Orchard 0.55.0 controller/worker and the cached digest-pinned Xcode 26.5 Tart image as task `task-20260820-0001-6ee0wg`. It synced 10 manifest-verified files, built successfully, passed two tests, launched `dev.archie.runner-fixture`, captured a 1206×2622 PNG, stopped at `archieDebugCheckpoint`, read `nonce = "ARCHIE_FULL_CYCLE_NONCE"`, collected an eight-frame backtrace, detached, captured a 19,523-byte `.ips` paired with the `ARCHIE_FULL_CYCLE_CRASH` unified-log marker, collected both `.xcresult` bundles, deleted the Simulator, and verified the exact backend was absent from Orchard and Tart inventories. The evidence manifest is versioned, records image/source/artifact hashes, and reports both cleanup assertions true.
- The first live attempt correctly failed and deleted its backend when it exposed a Node/Python path-order difference in the cross-host source manifest; both sides now sort POSIX path strings and emit expected/actual diagnostics. A second attempt proved build/test/debug/crash but correctly failed and cleaned up when ReportCrash exceeded the original 30-second wait. The final harness adds `simctl diagnose` recovery plus an explicit marker-bearing unified-log fallback and then passed with a genuine `.ips`.
- Froze the implementation in commit `a4ed5a12cc4e2032952f0d5374c54090d232ce0f` and reran that exact compiled code through a fresh isolated controller as task `task-20260820-0018-6woihe`. Evidence records the exact Archie commit, source commit `86ffd559f1f3b8ecb60afb9f65c43214fee10226`, 10-file manifest `401c6c005bc36c087f4e3292f78b8dbee06247c0864fa7d3c00791e96d813064`, and the digest-pinned Xcode image.
- Independent inspection of the collected bundles confirmed Xcode 26.5 build `17F42`, a successful arm64 Simulator build with zero warnings or errors, and two of two XCTest cases passed with no skips or failures. The visible 1206×2622 app PNG hashes to `de1acaa571085dfceeead7dcedb15ef8e7c9ca6ff37aabb7b84ea720411bd215`.
- The exact-commit LLDB transcript hashes to `c924265dfa2bfc987480c735b60fc64e226da132afed95bf1b4978ca54317de8` and contains the breakpoint stop, `nonce = "ARCHIE_FULL_CYCLE_NONCE"`, eight-frame backtrace, and clean detach. ReportCrash did not emit an `.ips` within the bounded collection interval on this repeat, so the explicit fallback retained a process-identified crash report and unified log containing `RunnerFixture/RunnerViewController.swift:49: Fatal error: ARCHIE_FULL_CYCLE_CRASH`; the report hashes to `a38fb6d22a43726fb05f0e49fe88d2505700e7ee2581f63608646aa6ae94f8e6`.
- Cleanup evidence reports both Simulator and backend deletion. Orchard logged the backend DELETE followed by a 404 and the worker's physical `tart delete`; subsequent Orchard inventory contained zero VMs and Tart contained only the two cached OCI image references. The isolated controller was then stopped and its temporary runner configuration removed.
- Final repository verification for the exact implementation passed `npm run typecheck`, `npm run build`, generated-project reproduction with XcodeGen, compiled canary import, `git diff --check`, and the complete Vitest suite: 86 files passed, 2 skipped; 1,181 tests passed, 6 skipped.
- At that point, a clean source build of the real Sweatcoin app still needed a prepared image/cache and approved private dependency access; the pinned-artifact canary was launch/debug evidence, not evidence of that dependency bootstrap.

## 2026-09-06 Simplification

- Rebased the branch onto `origin/main` at `15a5f9f` before changing the implementation.
- Removed the 547-line pinned-artifact Sweatcoin canary and its 26-line parser test. The deterministic source-to-debug canary supersedes its core acceptance purpose with a stronger build, XCTest, Simulator, LLDB, crash, artifact, and backend-deletion contract. The removed app-specific MP4 and MJPEG checks belong in the mobile deployment pipeline or repository workflow, not a second compiled Archie program.
- Removed the 300-line Orchard 0.56.1 transport patch, its 30-line build helper, and the patch-specific Git attribute. Production already requires the supported root-started worker with `--user`; retaining an unsupported, version-pinned network bypass increased security and maintenance surface without providing a production path.
- Consolidated repeated public-method lease acquisition in `RunnerManager` behind two explicit boundaries: `withReadyLease` provisions or recovers a ready lease before entering its keyed lock, while `withActiveLease` operates only on an existing lease under the same lock. Provisioning, persistence, capacity, cleanup, cursor delivery, output bounds, and archive containment are unchanged.
- Removed the agent-facing `runner_ensure` prewarm tool. `runner_sync` already provisions or recovers the lease and is required before execution, leaving one linear agent workflow without removing the manager's internal provisioning primitive.
- Added a responsibility map to the current architecture doc and marked the generated Xcode project as generated review material; `project.yml` remains its readable source.
- Kept prior lab results above as historical evidence. They describe what was tested at the time, not files or commands that remain supported by the branch.
- Reduced the branch from 7,221 to 6,297 added lines and from 54 to 50 changed files relative to `origin/main`, a net removal of 924 lines from the proposed change.
- Verification passed: TypeScript typecheck and build, 63 focused runner/contract tests with the real-Orchard case skipped, the complete suite with 1,640 tests passed and 6 skipped, the four-case configuration fail-fast smoke, and `git diff --check`. The disabled-mode Docker boot was unavailable because this checkout has no `.env`; the production image build was stopped while Docker waited on base-image registry metadata before executing any build layer. The destructive real-Orchard tier was not requested.
