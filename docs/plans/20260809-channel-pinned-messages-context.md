...
| AC2 | WHEN pins.list fails for any reason including missing_scope, THEN no pins block is emitted for a channel with nothing stored, any previously stored pins are left untouched, the task spawns normally, and nothing throws. | unit |
...
## What changed between this plan and what shipped

This plan was signed before implementation and is left as it was signed, because it is the contract the operator agreed to. Review and QA then moved seven things, each for a reason worth recording. Where the plan and this list disagree, this list is what shipped.

**`pinsTotal` is `pinsEligible`, and it counts something else.** The plan stored the pre-cap count of everything Slack reported. The shipped field is the count of pins that PASSED the trust gate, because the omission disclosure computed from a pre-gate total told every agent there was context it could not see about pins the gate had deliberately refused — and leaked how many externally-authored pins a channel had.

**The cap runs after the trust gate, not before it.** As planned, capping first let refused pins consume index slots: one external participant pinning twenty-five things blanked a channel's index entirely, which anyone could have caused by accident.

**Containment is XML escaping, not tag stripping.** The plan specified stripping `</pin>` and `</channel_pinned_messages>` out of the text and escaping attributes with `JSON.stringify`. Adversarial QA broke that three ways — only closing tags were neutralised, so a body could forge a whole `<pin by="someone else">` element and let the genuine closer terminate it; `JSON.stringify` escapes a quote as a JSON escape, which means nothing in tag syntax, so a display name could still end an attribute; and the invisible-character blocklist it relied on caught 8 of 197 candidate codepoints. Every value now goes through one XML escape covering `&`, `<`, `>` and `"`, in attributes and element text alike, which holds without enumerating attacks.

**Pin text comes from the full extraction, not `message.text`.** The plan reasoned from Slack's documented `pins.list` example that message items carry `text` only, and concluded the index was cheap for that reason. In practice `text` is the legacy fallback: app posts, workflow output and unfurl cards — among the most-pinned things in a real channel — arrive with it empty and would each have indexed as a blank line. Pinned payloads now go through the same batched `resolveRawMessages` extraction as every other inbound path, which also means an app-posted pin reports no human author and is refused outright.

**Model calls are budgeted per scan.** Not in the plan at all. The scan is awaited before the PM wakes, so a first scan of a long-pinned channel would have run up to twenty-five sequential subprocess spawns while a user waited. At most six run per scan; the overflow is indexed from its own truncated text, carries a sentinel digest, and is upgraded on a later scan.

**Each `<pin>` carries `source` and `channel_id`.** Neither was planned. The block's note claimed every line was a summariser's paraphrase, which was false for exactly the lines that matter — a pin under the verbatim threshold is rendered byte for byte, and a pinned file's line is its title, so the note was describing user-typed text as machine-written and lowering the agent's guard on it. And the note told the agent to open a pin using a channel value the block never carried.

**The index appends above the canvas block, so it reads first.** The plan asserted the opposite in one place while its placement instruction implied this; the two could not both hold. The placement is kept — it stays clear of the hunks open PRs anchor below the canvas block — and the assembled prompt reads index first, standing brief second, which is the better order anyway: the brief is the authoritative one and belongs nearest the instructions that follow.
...
| AC2 | unit | listChannelPins is stubbed to return null (both the generic-error and missing_scope paths are exercised in client.test.ts); ensureChannelPins runs against a channel with no store and against one holding pins, and buildChannelPinsPromptSection is called for each. | Passing assertions that updateChannelStore was not called, the pre-existing store is unchanged, the section is '' for the empty-store channel and still renders the retained pins for the populated one, and no error propagated; plus the client.test.ts case showing missing_scope warns once and suppresses all later pins.list calls. | `npx vitest run src/connectors/slack/__tests__/channel-pins.test.ts src/connectors/slack/__tests__/client.test.ts` |
