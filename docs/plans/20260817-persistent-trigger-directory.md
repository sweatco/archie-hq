...
- AC13 was signed as "the announcement block names the path AND directs the agent to load the trigger-task skill", and the plan additionally amended two other prompt sites: the PM's `Spawned by trigger:` context line and the seed message. That came to three places asserting some version of "you were started by a trigger", and they drifted almost immediately. Consolidated after the operator reviewed the shipped prompts: the seed message names the skill, the announcement block names the trigger and its directory, and the PM context line is gone. AC13 is restated as "announced in exactly one prompt block, on every track" — the skill instruction moved rather than disappeared.
