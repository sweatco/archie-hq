...
| AC4 | WHEN a trigger-fired agent is spawned, THEN the directory is in BOTH allowWritePaths and allowReadPaths, the shape every other read-write path uses | unit |
...
### T06 — WITHDRAWN — no longer needed once the grant covers both lists

Withdrawn before merge. This task widened `assertReadable` to accept `allowWritePaths` entries, which was only necessary because the directory was granted write-only. The write-only form existed to avoid a bwrap downgrade that was then measured not to happen (see the notes), so the grant now lists the path in both sandbox lists like every other read-write path, and `src/agents/artifacts.ts` is untouched by this change — identical to `main`. The test file this task added is kept, repurposed to pin the actual behaviour: a path in `allowWritePaths` alone is NOT readable by the artifact tools. That is the constraint which decides how the grant has to be shaped, and it also documents the limitation `CACHES_DIR` lives with.

**Files:** `src/agents/__tests__/artifacts.test.ts`

**Tests:** npx vitest run src/agents/__tests__/artifacts.test.ts
...
| AC3 | structural | Confirm the spawn.ts block is guarded on metadata.triggered_by, so a task without it creates nothing and is granted nothing. | The grep prints the ternary guard line; grantTriggerDataAccess and ensureTriggerDataDir appear only inside that guarded region. | `grep -n -A4 'const triggerDataPath = metadata.triggered_by' src/agents/spawn.ts && test "$(grep -c 'grantTriggerDataAccess(' src/agents/spawn.ts)" = 1` |
| AC4 | unit | Call the grant function with a SandboxOptions carrying populated read and write lists, then inspect both. The read grant is what lets the in-process artifact tools share a file the agent wrote there; assertReadable validates against allowReadPaths alone. | Passing assertions in src/agents/__tests__/trigger-data.test.ts that the path is appended to both lists, that every other option is unchanged, and that the input is not mutated; plus the write-only case in src/agents/__tests__/artifacts.test.ts showing why write-only would not do. | — |
...
- AC13 was signed as "the announcement block names the path AND directs the agent to load the trigger-task skill", and the plan additionally amended two other prompt sites: the PM's `Spawned by trigger:` context line and the seed message. That came to three places asserting some version of "you were started by a trigger", and they drifted almost immediately. Consolidated in two passes after the operator reviewed the shipped prompts. First pass: the PM context line deleted, the skill instruction moved to the seed, the directory named only in the block. Second pass: the skill instruction moved back into the block, because the seed reaches the PM alone while a delegated repo or plugin agent holds the same directory — the block is the only trigger-specific text every track sees. `knowledge.log` was considered for that job and rejected: it is the data channel the injection rules tell agents to read as data, and a line written at fire time reads as stale history rather than a standing instruction. AC13 stands as written; what changed is that the block is now the *only* place either fact appears.
- The seed's closing "there is no prior conversation" was replaced rather than amended. The plan's amendment (T05) bolted the directory onto that sentence, which made three prompt sites mention the directory and still left the misleading half standing — a trigger that has run before does leave context. The sentence's real content was that nobody is waiting on the other end, which is what it now says, and the directory is named once, in the block.
- The prompts also named tools — `Read`, `Write`, `Edit`, `ls`, `ls -R`, and the note that `Glob` is absent. All removed from the block and from the skill, on the operator's instruction: an agent already knows how to read a file and list a directory, and naming a tool dates the prompt to one runtime's tool set. The `Glob` incident above is the argument for that rule rather than against it — the damage came from a prompt naming a tool the runtime did not have.

- AC4 was signed as "the directory is in allowWritePaths and NOT in allowReadPaths". That criterion rested on the bwrap claim above, which is false: listing a path in both lists is measurably identical to listing it in allowWrite alone. Since write-only bought nothing and cost something — assertReadable checks allowReadPaths alone, so an agent could write a file it could not then share — the grant now uses both lists, the shape every other read-write path uses, and AC4 is restated to match. The knock-on is that T06, which widened assertReadable, is withdrawn: src/agents/artifacts.ts is untouched by this change, which also removes the CACHES_DIR side effect that widening introduced.
