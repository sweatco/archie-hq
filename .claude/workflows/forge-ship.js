...
1. Record the plan: if a docs/plans/*-${input.change || 'change'}.md record already exists on this branch (a previous ship attempt), update it in place — never create a second record for the same change. Otherwise write docs/plans/$(date +%Y%m%d)-${input.change || 'change'}.md per that folder's README convention — a status header ("Status: Implemented (this PR)"), then the brief, the acceptance criteria with their methods, the design, the tasks, and the verification plan. Add its row to the Implementation Status table in docs/plans/README.md (once). Never hard-wrap prose. Commit on ${input.branch}.
${input.plan ? `The plan to record:\n${JSON.stringify({ design: input.plan.design, tasks: input.plan.tasks, verificationPlan: input.plan.verificationPlan }, null, 2)}\n` : 'No plan object was provided — skip step 1 and say so in notes.\n'}2. Push: git push -u origin ${input.branch} (on network failure retry up to 4 times with exponential backoff: 2s, 4s, 8s, 16s).
