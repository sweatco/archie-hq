...
- **Same session as the run:** launch with `args: {change, branch, base, brief, acs, plan, request, evidenceDir, workflowsDir}` — everything except `request` (the operator's words, verbatim) comes from the run's `done` result.
- **Different session (the run's session is gone):** reconstruct the identity from durable artifacts — brief, ACs, design/tasks, and verification plan from the run's `docs/plans/*-<change>.md` record on the branch; manifest context from the PR body. The plan record exists precisely so revision never depends on the original session.
- QA failures after a revision impasse back to the operator (no auto route-back — they are steering). Answer keys work as in forge-run: `answers.implement` (keyed map; the revision itself is keyed `request`), `answers.qa`, `answers.docs`, `answers.ship`.
