...
On `done`: report the PR link and render the verification manifest (per-AC: criterion, method, status, evidence — waivers stated plainly). State the loop outcomes in plain words from `rounds` — e.g. "plan accepted in 1 critique round, implementation clean in 1 review round, QA passed in 1 cycle, docs verified in 1 round" — and list every entry of `nonBlockingFindings` (they required no fix, but the operator must see them, not discover them in a verdict object). This is the context that makes the progress display legible: completed reviewers with no fixer agents after them means zero blocking findings, and the fail-closed AGENT-FAILURE rule guarantees a dead reviewer cannot masquerade as a pass. Subscribe to the PR's activity per the session's normal PR machinery; keep the `done` result's `plan` for the session's lifetime — post-ship changes of every kind (operator steers, CI failures needing code changes, substantive review feedback) go through **Revise** below, never through hand-edits. The merge decision is the user's, on GitHub. If they ask post-merge: file follow-up issues for every waived AC with a real post-merge step.

## Revising a shipped run (`/forge revise <request>`)

When the operator requests a change to a PR that a Forge run produced — their own steer, or review feedback they endorse — **never hand-edit the run's branch.** Every post-ship code change goes through `forge-revise.js` (by scriptPath, per the ground rule), which chains request-mode implement (blind reviewers included, told the request is sanctioned scope on top of the plan) → one full QA pass → docs delta → PR-and-plan-record refresh, and returns the same done/impasse contract as `forge-run`. Only PR-description-only edits may be made directly.

- **Same session as the run:** launch with `args: {change, branch, base, brief, acs, plan, request, evidenceDir, workflowsDir}` — `plan` comes from the run's `done` result; `change`/`branch`/`base`/`brief`/`acs` are the run's own launch parameters you still hold; `request` is the operator's words, verbatim.
- **Different session (the run's session is gone):** reconstruct the identity from durable artifacts — brief, ACs, plan summary, design, tasks, and verification plan from the run's `docs/plans/*-<change>.md` record on the branch; manifest context from the PR body. The plan record exists precisely so revision never depends on the original session. The workflow validates the plan shape up front and refuses a reconstruction missing design/tasks/verificationPlan — fix the reconstruction, don't force it.
- **A failing QA after a revision is terminal for that launch** — resuming would replay every completed call from cache into the identical failure. The escape is a FRESH `forge-revise` launch with an amended request (the branch keeps the previous revision's commits), or abandoning the revision. The `answers.*` keys serve the OTHER impasses only: `answers.implement` (keyed map; the revision itself is keyed `request`, plus `setup`/`gate`/`review`), `answers.qa` (runner/auditor death, not failing ACs), `answers.docs`, `answers.ship`.
- On `done`: report the updated PR, the refreshed manifest, rounds, and non-blocking findings, exactly as for a run.
...
1. **Launch** `forge-review.js` (by scriptPath, per the ground rule) with `args: {pr, qaOnly, scratchDir}` and `scratchDir` under the session scratchpad — or, for branch mode, `args: {branch: true, base, intent, qaOnly, scratchDir}` instead of `pr`. The workflow grounds itself (worktree + fact-checked lenses, including a **direction lens** over open issues/PRs, `docs/proposals/`, and `docs/plans/`), derives intent and numbered ACs from the PR autonomously (assumptions flagged, the PR's own "couldn't verify" admissions become ACs), runs the review ring (skipped in `qa-only`) and the blind QA ring, tears down its worktree, and returns `{intent, assumptions, acs, reviewFindings, scopeQuestions, previousFindingRulings, qaManifest, gaps, recommendation}`. It never impasses — dead agents, unavailable infra, and skipped rings surface as `gaps` in the report; the only two unrecoverable failures (no worktree, no derivable ACs) return `status: 'error'`, and even those run the teardown first. It never commits, pushes, or posts.
2. **Report in chat**, one message: derived intent + ACs with every assumption flagged, the per-AC verdict table with evidence, review findings ranked by severity (CONFIRMED before PLAUSIBLE), **the scope questions as explicit questions to the operator** (each with its evidence: unused abstraction, uncommitted-future groundwork, direction collision — these are the descope calls only the operator can make; ask them, don't decide them), the gaps verbatim, and the recommendation (approve / needs-discussion / request-changes). Nothing has touched GitHub yet — say so.
3. **Iterate**: the user corrects assumptions or ACs, and **answers the scope questions** — their scope rulings become part of the review content (a descope request, or dropped) → relaunch with `args.corrections` (their words, verbatim) when the analysis itself needs redoing; update the report. Repeat until they're satisfied.
4. **Submit on approval only.** On the explicit go, post the review via the GitHub MCP (pending review → line-anchored comments for findings with `file:line`; operator-approved scope requests go in the review summary). Attribute honestly: the review states it is an Archie/Forge review. Then stop — the author handles the outcome.
