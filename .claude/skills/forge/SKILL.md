...
5. **Follow-up round** (same session, when the author pushes fixes): relaunch with `args.sinceSha` (the previously reviewed head, from the last result) and `args.previousFindings` (the findings you reported) — the workflow reviews the delta and returns `previousFindingRulings`, one fixed / unaddressed / regressed ruling per prior finding. Follow-up rounds always run the full review ring (`qaOnly` is ignored — the rulings live there). Submitting again requires a fresh go.
