...
Then reconstruct the brief from the PR **and** the dossier: description (What & why / Verification sections), linked issues, review comments, and what the code actually shows. Mark every reconstructed item as **inferred**; where the dossier contradicts the PR's claims, surface the contradiction explicitly in the brief. Ask "what would make you comfortable merging this?" — their answer becomes ACs. Pay special attention to the PR's own "couldn't verify" admissions: each becomes an AC with method `live-e2e` or an explicit `deploy-only` waiver.

There is **no separate "confirm the inferred items" step** — confirmation of the reconstruction happens at the exit gate below, and only there. Do not ask the user to approve, confirm, or correct the brief until you are rendering it in full per the exit criteria; asking for confirmation of a brief the user hasn't seen in the chat is the exact failure this rule exists to prevent.
