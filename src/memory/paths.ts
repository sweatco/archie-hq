...
/** Maximum number of full non-`org` entity pages injected into a single agent prompt (`0` → index-only for non-org pages). */
...
/** Maximum `touched_by` relations rendered into an injected entity block (newest kept; `0` → none). Render-time only — the stored page keeps the full history. */
