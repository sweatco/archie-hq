...
export async function writePrivateOutcome(
  scope: Extract<TaskMemoryScope, { kind: 'channel' | 'user' }>,
  ...
  if (!Number.isFinite(createdAtMs) || !summary) return false;
  ...
  const next = [
    ...
    .sort((a, b) => b.created_at.localeCompare(a.created_at) || a.task_id.localeCompare(b.task_id))
