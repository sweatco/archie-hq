...
- **Live authorization.** Memory reads and Slack deliveries reclassify the fixed destination while memory is active. Lookup failures and external/guest visibility deny scoped access. With memory disabled or unavailable, normal delivery still works inside the fixed destination.
