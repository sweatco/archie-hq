...
/**
 * The single declaration of which core skills each track mounts: the four PM-voiced skills on the PM alone, and `trigger-continuity` — which every agent on a trigger-fired task needs — on all three tracks.
 *
 ...
 *
 * Content limit: the four PM-only skills are written in the PM's voice and instruct tools attached only inside the `isPmAgent(def)` branch that begins at `src/agents/spawn.ts:325` — `comms-tools` and `orchestration-tools` are registered at `:398-400`, giving the PM alone `post_to_channel`, `mute_channel`, `read_thread`, `fetch_slack_reference` and `report_completion`, all of which those four instruct by name. `skills/self-awareness/SKILL.md:53` even describes itself as a built-in PM skill. So giving one of them a new audience needs prompt and tool work too, not just a line here. `trigger-continuity` can sit on `repo` and `plain` only because it was deliberately written for that audience: track-neutral prose that assumes no channel, no users and no teammates, and names none of those five tools — none of any MCP tool, in fact. That is the bar a future core skill added to a non-PM track has to clear.
 */
