...
export async function spawnAgent(agent: Agent, task: Task): Promise<void> {
  ...
  if (isPmAgent(def)) {
    ...
    if (metadata.triggered_by) {
      contextLines.push(`Spawned by trigger: ${metadata.triggered_by} (this is a fresh, trigger-initiated task — deliver the result as instructed in the first message; do not create new triggers from here)`);
    }
