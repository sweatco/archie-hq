...
describe('task metadata memory persistence', () => {
  ...
  it('keeps the immutable destination when stale Task instances save concurrently', async () => {
    ...
    const publicTask = new TaskCtor(taskId, metadata(taskId), PM_DEF);
    const noneTask = new TaskCtor(taskId, metadata(taskId), PM_DEF);
  ...
  it('merges memory authors from stale writers under the metadata lock', async () => {
    ...
    await Promise.all([
      new TaskCtor(taskId, first, PM_DEF).save(true),
      new TaskCtor(taskId, second, PM_DEF).save(true),
    ]);
  ...
  it('persists the inbound destination and keeps it after restart', async () => {
    ...
    await seed(taskId, metadata(taskId));
    memoryReady = false;
    const firstProcess = new TaskCtor(taskId, metadata(taskId), PM_DEF);
    ...
    memoryReady = true;
    const restarted = new TaskCtor(taskId, afterFirstIngest, PM_DEF);
