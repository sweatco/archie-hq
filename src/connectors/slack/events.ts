...
export async function mountSlackApp(
  ...
  // Handle the per-user MCP OAuth wall button. Whoever clicks becomes the
  // acting user for that (task, server): Slack authenticates the clicker
  // (body.user.id), and the authorize URL is delivered ephemerally to them
  // only — never posted into the shared thread. The wall message itself stays
  // intact (button included) so someone else can still volunteer if the
  // clicker abandons; completion edits it in place.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  app!.action('authorize_mcp', async ({ action, ack, body }: any) => {
    await ack();
    ...
    let payload: { taskId?: string; authRequestId?: string; server?: string } = {};
    try {
      payload = JSON.parse(action.value);
    } catch {
      // fall through to the guard below
    }
    const { taskId, authRequestId, server } = payload;
    if (!userId || !taskId || !authRequestId || !server) {
      logger.error('Server', `authorize_mcp click with malformed payload: ${action.value}`);
      return;
    }

    const tellUser = async (text: string): Promise<void> => {
      if (channelId) await postEphemeral(channelId, userId, text, threadTs);
    };

    logger.server(`MCP authorization click by ${userId} for "${server}" (task ${taskId})`);

    try {
      // The acting user must be a home-workspace member: an external or guest
      // account binding its provider identity to an internal task is exactly
      // the confused-deputy shape the redaction pipeline exists to prevent.
      const clicker = await getUserInfo(userId);
      if (isExternalUser(clicker)) {
        await tellUser('Sorry — external or guest users can\'t authorize MCP access for this workspace.');
        return;
      }

      const task = await Task.get(taskId);
      const outcome = await task.handleMcpAuthClick({
        authRequestId,
        server,
        slackUserId: userId,
        channelId,
      ...
      switch (outcome.kind) {
        case 'stale':
          await tellUser('This authorization request is no longer active. If access is still needed, the task will ask again.');
          break;
        case 'already_bound':
          await tellUser(`"${server}" was already authorized by <@${outcome.user}> for this task.`);
          break;
        case 'connected_existing':
          await tellUser(`Connected — "${server}" is now using your existing authorization. The task resumes automatically.`);
          break;
        case 'url':
          await tellUser(
            `Authorize *${server}* with your own account:\n${outcome.url}\n\n` +
            'This link is for you only, single-use, and expires in 1 hour.',
          );
          break;
      }
    } catch (error) {
      logger.error('Server', 'Error handling MCP authorization click', error);
      const reason = error instanceof Error ? error.message : String(error);
      await tellUser(`Authorization setup for "${server}" failed: ${reason}`).catch(() => {});
    }
  });
