...
**Status:** Target spec — reflects shipped behavior through the entity layer, bounded relevance-gated injection, the agent-callable read tools, the channel-confidentiality authorization policy, and the eval harness (see the archived changes for history)
...
Both directions are governed by a channel-confidentiality policy: tasks that touched a private, externally-shared, or unclassifiable Slack channel store nothing; DM-carrying tasks write user preferences ONLY (no episodic artifacts of any kind — DM content is never readable by any other task, while DM-task callers keep the full org read surface); a user's memory is written from and served to only that user's own evidence-cited statements; and tasks visible to an external org — or possibly visible (classification failure) — get no memory surface at all.

## Non-Goals

These are explicitly out of scope for this capability. Each is a future enhancement that requires its own proposal.

- **Retroactive store cleanup.** The confidentiality policy (see "Channel visibility classification", "Extraction confidentiality gate", and the read-tools authorization) applies from its ship date forward; pre-policy artifacts on disk are read-denied via the missing `access:` stamp (first-iteration `access: dm` stamps deny identically) and dropped from activity injection, not deleted. An automated cleanup script over dev stores is optional, not a blocker.
- **Per-fact entity provenance.** Entity pages are org-visible by construction — the extraction gate keeps private-channel content out of them entirely, and DM-sourced entity facts are org-visible by policy. Tagging individual observations with source visibility is deferred until evidence (store review, denial telemetry) shows confidential facts landing on pages.
- **Channel rosters.** Authorization keys on stamped channel visibility, stable DM channel ids, and transcript authorship — never on Slack membership lists (`conversations.members` is not consulted).
...
- **Channel visibility** — the per-Slack-channel class `public | private | dm | ext-shared`, stamped onto task metadata from `conversations.info` (fail-closed to `private` on error or when never stamped).
- **Access class** — the per-task grant stamped into summary frontmatter at extraction. `org` — the only class extraction writes — marks artifacts readable by every agent. DM-carrying tasks extract prefs-only (no episodic artifacts, so no class); gated tasks store nothing. A first-iteration `access: dm` stamp is denied like a missing stamp.
- **Author users** — the users who actually posted messages in a task's transcript (source-line scan), as opposed to users merely @-mentioned. Authorship is the memory-ownership boundary.
## Requirements
...
Removal of the entire memory capability SHALL require only setting `ARCHIE_MEMORY=false` (immediate disable) OR deleting `src/memory/`, `prompts/memory-extractor.md`, the call sites in `src/index.ts` and `src/agents/spawn.ts`, plus `workdir/memory/`. No database migrations, no external service cleanup, no type changes to non-memory modules (the `ChannelVisibility` stamp on channel records is core metadata that outlives ejection). Core modules MUST NOT import from `src/memory/` except through the public surface (`initMemory`, `enrichPromptWithMemory`, `isMemoryEnabled`, `isInjectionEnabled`, `isMemoryToolsEnabled`, `createMemoryToolsMcpServer`, and the `MemoryToolsCtx` type).
