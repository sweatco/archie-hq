...
- **Roadmap committed.** The re-cut plan for the remaining phases — eval harness promoted to the gate for enabling injection in prod, Branch-A spike + read tools moved ahead of embeddings, semantic dedupe split out from embedding-backed selection — is captured as a concise `docs/proposals/memory-v2-roadmap.md`. The full research/decision doc (`docs/proposals/memory-v2.md`) stays untracked by choice.
...
- **Docs (same change, per `src/memory/CLAUDE.md`):** `docs/architecture/memory.md` (selection semantics + flags table), `.env.example` (new flag), `docs/proposals/memory-v2-roadmap.md` (re-cut phase plan).
