...
- [ ] 6.2 Flip `ARCHIE_MEMORY_TOOLS=true` with or after the injection flip (design D8), own PR carrying the first full `memory:eval` report; optionally verify on a booted branch instance first via the `archie-e2e` harness (seed store → task → assert pull records)
