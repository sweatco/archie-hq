...
- **Coexists with the shared model.** Injection precedence: the acting user's per-user token wins → falls back to a shared operator token if present → else wall. Existing shared connections keep working unchanged.
...
- **New code:** `request_mcp_auth` in-process tool; the "Authorize" Slack button action handler; daemon-side `beginConnect` trigger; per-user token storage layout and injection resolution; acting-user binding in `TaskMetadata`; needs-auth classification/cache per server.
- **Modified code:** `src/system/oauth/storage.ts` (per-user + shared-client layout), `src/system/oauth/inject.ts` (acting-user resolution + precedence), `src/system/oauth/connect.ts` (invocable in-daemon), `src/connectors/oauth/routes.ts` (`slack_user_id` in the pending → callback → vault chain), `src/agents/spawn.ts` (inject acting user's token; prompt-inject requestable servers), `src/connectors/slack/events.ts` (wall button handler), `src/agents/tools.ts` (new tool), `src/tasks/task.ts` (park/wake + binding), `src/types/task.ts` (binding field), `src/types/*` for OAuth records.
